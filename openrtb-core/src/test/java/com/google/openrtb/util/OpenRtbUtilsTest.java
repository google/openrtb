...
public class OpenRtbUtilsTest {
  ...
  public void testAddFilter_specialCases() {
    Predicate<Imp> pred = Predicates.<Imp>alwaysTrue();
  ...
  public void testResponse_remove() {
    ...
    OpenRtbUtils.removeBids(response, Predicates.<Bid.Builder>alwaysTrue());
    assertThat(OpenRtbUtils.bids(response)).hasSize(3);
    assertThat(OpenRtbUtils.removeBids(response, new Predicate<Bid.Builder>() {
      @Override public boolean apply(Bid.Builder bid) {
    ...
    assertThat(OpenRtbUtils.removeBids(response, "x", new Predicate<Bid.Builder>() {
      @Override public boolean apply(Bid.Builder bid) {
        return !"4".equals(bid.getId());
      }})).isTrue();
    assertThat(OpenRtbUtils.bids(response, "x")).hasSize(1);
    assertThat(OpenRtbUtils.removeBids(response, "none", Predicates.<Bid.Builder>alwaysFalse()))
        .isFalse();
    assertThat(OpenRtbUtils.removeBids(response, null, Predicates.<Bid.Builder>alwaysFalse()))
        .isTrue();
    assertThat(OpenRtbUtils.removeBids(response, "x", Predicates.<Bid.Builder>alwaysFalse()))
        .isTrue();
    assertThat(OpenRtbUtils.bids(response, "x")).isEmpty();
    assertThat(OpenRtbUtils.removeBids(response, Predicates.<Bid.Builder>alwaysFalse())).isFalse();
    assertThat(OpenRtbUtils.bids(response)).isEmpty();
  }
