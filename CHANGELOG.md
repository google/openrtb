...
* THE BIG MOVE TO JAVA 8! The library now requires JDK 8, and takes
  ...
  - `ProtoUtils` and `OpenRtbUtils`: parameters with `Function` or
    ...
    but needs no changes. Some utility methods have new variants using
    streams, for example `OpenRtbUtils.bidStreamWith()`; if you like
    streams, the new methods are easier and often more efficient than
    calling the old methods and doing your own conversion to a `Stream`.
